...
def create_default_salle_and_assign(apps, schema_editor):
    ...
    Utilisateur.objects.filter(salle__isnull=True).update(salle_id=salle.id)
...
def reverse(apps, schema_editor):
    ...
    Utilisateur = apps.get_model('users', 'Utilisateur')
    Utilisateur.objects.filter(salle_id=salle.id).update(salle=None)


class Migration(migrations.Migration):

    dependencies = [
        ('users', '0006_add_salle_nullable'),
        ('salles', '0001_initial'),
    ]
