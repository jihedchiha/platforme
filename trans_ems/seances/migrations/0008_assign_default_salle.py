...
def create_default_salle_and_assign(apps, schema_editor):
    ...
    Seance = apps.get_model('seances', 'Seance')
    Seance.objects.filter(salle__isnull=True).update(salle_id=salle.id)
...
def reverse(apps, schema_editor):
    ...
    Seance = apps.get_model('seances', 'Seance')
    Seance.objects.filter(salle_id=salle.id).update(salle=None)


class Migration(migrations.Migration):

    dependencies = [
        ('seances', '0007_add_salle_nullable'),
        ('salles', '0001_initial'),
    ]
